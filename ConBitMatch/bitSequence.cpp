...
{
	int timewindowvalue = a.timeWindow/timeSlice +1;
	std::vector<BitWordWithID> bwArray;
	struct attcons
	{
	...
	int tcflag = 0;
	int unitflag = 0;
	size_t changeBit = 0x4000000000000000;
	int tsnum;
	for (int i = 0; i < procData.tag.size(); i++)
	{
			...
			if (consnum != 0)
			{
				for (int j = 0; j < constxt[consnum - 1].attName.size(); j++)
				{
					...
					if (j == constxt[consnum - 1].attName.size() - 1)
					{
						...
						if (cor == a.pt.evtype[a.pt.evtype.size() - 1])
						{
							if (bwArray.size() <= unitflag)
							{
								gggggg = 1;
								bwArray.push_back({ unitflag, {false, {Layer{positionnum,changeBit}}} });
							}
							else
							{
							bool flagaddlayer = true;
							for (auto& layer : bwArray[unitflag].bitWord.layers) {
								if (positionnum - layer.value > timewindowvalue)
								{
									flagaddlayer = false;
								
									layer.binary_val |= changeBit;
									layer.value = positionnum;
									break;
								}
							}
							if (flagaddlayer) {
								bwArray[unitflag].bitWord.layers.emplace_back(positionnum, changeBit);
								bwArray[unitflag].bitWord.flag = true;
							}
						}

					}
						if (gggggg == 0)
							bwArray.push_back({ unitflag, {false, {Layer{-1,0}}} });
